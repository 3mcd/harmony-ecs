...
import { formats, makeSchema } from "./model"
...
describe("applyDeferredOps", () => {
  it("applies make operations to a world", () => {
    ...
    expect(world.archetypeRoot.edgesR[0].entities).toHaveLength(1)
    expect(world.archetypeRoot.edgesR[0].table[0].data[0]).toBe(99)
  })
  it("applies set operations to a world", () => {
    ...
    deferSet(manager, entity, Schema, 99)
    applyDeferredOps(world, manager)
    expect(world.archetypeRoot.edgesR[0].entities).toContain(entity)
    expect(world.archetypeRoot.edgesR[0].table[0].data[0]).toBe(99)
  })
  it("applies unset operations to a world", () => {
    ...
    deferUnset(manager, entity, Schema)
    applyDeferredOps(world, manager)
    expect(world.archetypeRoot.edgesR[0].entities).not.toContain(entity)
  })
