...
describe("add", () => {
  it("refuses an abnormal type", () => {
    ...
    expect(() => Type.add(type, 0 as SchemaId)).toThrowError("abnormal type")
  })
  it("prepends a type to a normal type", () => {
    const type = [1, 2, 3] as SchemaId[]
    expect(Type.add(type, 0 as SchemaId)).toEqual([0, 1, 2, 3])
  })
  it("inserts a type within a normal type", () => {
    const type = [0, 1, 3] as SchemaId[]
    expect(Type.add(type, 2 as SchemaId)).toEqual([0, 1, 2, 3])
  })
  it("appends a type to a normal type", () => {
    const type = [0, 1, 2] as SchemaId[]
    expect(Type.add(type, 3 as SchemaId)).toEqual([0, 1, 2, 3])
  })
})
...
describe("remove", () => {
  it("refuses an abnormal type", () => {
    const type = [2, 1, 0] as SchemaId[]
    expect(() => Type.remove(type, 0 as SchemaId)).toThrowError("abnormal type")
  })
  it("shifts a type from a normal type", () => {
    const type = [0, 1, 2, 3] as SchemaId[]
    expect(Type.remove(type, 0 as SchemaId)).toEqual([1, 2, 3])
  })
  it("splices a type from a normal type", () => {
    const type = [0, 1, 2, 3] as SchemaId[]
    expect(Type.remove(type, 2 as SchemaId)).toEqual([0, 1, 3])
  })
  it("pops a type from a normal type", () => {
    const type = [0, 1, 2, 3] as SchemaId[]
    expect(Type.remove(type, 3 as SchemaId)).toEqual([0, 1, 2])
  })
})
...
describe("maybeSupersetOf", () => {
  ...
  it("matches type where outer type may be subset of eventual superset of inner type", () => {
    expect(Type.maybeSupersetOf([0] as SchemaId[], [1, 2, 3, 4] as SchemaId[])).toBe(true)
    expect(Type.maybeSupersetOf([7, 9] as SchemaId[], [9, 10] as SchemaId[])).toBe(true)
    expect(Type.maybeSupersetOf([5, 6, 7, 8] as SchemaId[], [6, 99] as SchemaId[])).toBe(
      true,
    )
  })
  it("fails to mach type where outer type may never be subset of superset of inner type", () => {
    expect(Type.maybeSupersetOf([0, 5] as SchemaId[], [4, 5, 6] as SchemaId[])).toBe(
      false,
    )
    expect(Type.maybeSupersetOf([9, 10] as SchemaId[], [7, 9] as SchemaId[])).toBe(false)
    expect(Type.maybeSupersetOf([5] as SchemaId[], [6, 12] as SchemaId[])).toBe(true)
  })
})
...
describe("getIdsBetween", () => {
  ...
  it("builds an array of type ids between inner and outer", () => {
    expect(
      Type.getIdsBetween([0, 1, 4, 9, 12, 17] as SchemaId[], [9, 17] as SchemaId[]),
    ).toEqual([0, 1, 4, 12])
    expect(Type.getIdsBetween([0, 1, 2] as SchemaId[], [1] as SchemaId[])).toEqual([0])
    expect(Type.getIdsBetween([0, 1, 2, 3] as SchemaId[], [3] as SchemaId[])).toEqual([
      0, 1, 2,
    ])
  })
})
